...
const MobileNav = ({ active, onNavigate, allowedKeys, onLogout }: MobileNavProps) => {
  const items = allowedKeys ? navItems.filter((n) => allowedKeys.includes(n.key)) : navItems;
  return (
    <nav className="xl:hidden fixed bottom-4 left-1/2 -translate-x-1/2 w-[calc(100%-32px)] max-w-3xl bg-white/90 backdrop-blur-xl text-text shadow-card rounded-xl px-3 py-2 flex items-center gap-2 border border-borderSoft">
      ...
      {onLogout && (
        <button
          onClick={onLogout}
          className="h-9 px-3 rounded-lg bg-accent text-white text-xs font-semibold shadow-soft hover:shadow-card transition"
        >
          Cerrar
        </button>
      )}
